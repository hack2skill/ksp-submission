import os
import random
from .forms import ScrapForm, CombinedForm
...
from .medium import scrapMedium
from .twitter import scrapTwitter
from .instagram import scrapInstagram
from .github import scrapGithub
from .pinterest import scrapPinterest
from .quora import scrapQuora
from .linkedin import scrapLinkedin
from .combined import finaldata
from FacebookPostsScraper.facebook import scrapFacebook
...
def instagramscrap(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            scrapInstagram(un)
            return render(request, 'instagramscrap.html',context={'form':form})
        return render(request, 'instagramscrap.html',context={'form':form})
    return render(request, 'instagramscrap.html',context={'form':form})

def twitterscrap(request):
    form = ScrapForm(request.POST)
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            un = form.cleaned_data['username']
            scrapTwitter(un)
            return render(request, 'twitterscrap.html',context={'form':form})
        return render(request, 'twitterscrap.html',context={'form':form})
    return render(request, 'twitterscrap.html',context={'form':form})

def linkedinscrap(request):
    form = ScrapForm(request.POST)
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            un = form.cleaned_data['username']
            scrapLinkedin(un)
            return render(request, 'linkedinscrap.html',context={'form':form})
        return render(request, 'linkedinscrap.html',context={'form':form})
    return render(request, 'linkedinscrap.html',context={'form':form})

def githubscrap(request):
    form = ScrapForm(request.POST)
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            un = form.cleaned_data['username']
            scrapGithub(un)
            return render(request, 'githubscrap.html',context={'form':form})
        return render(request, 'githubscrap.html',context={'form':form})
    return render(request, 'githubscrap.html',context={'form':form})

def facebookscrap(request):
    form = ScrapForm(request.POST)
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            un = form.cleaned_data['username']
            scrapFacebook(un)
            return render(request, 'facebookscrap.html',context={'form':form})
        return render(request, 'facebookscrap.html',context={'form':form})
    return render(request, 'facebookscrap.html',context={'form':form})

def quorascrap(request):
    form = ScrapForm(request.POST)
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            ...
            print("Medium")
            scrapQuora(un)
            return render(request, 'quorascrap.html',context={'form':form})
        return render(request, 'quorascrap.html',context={'form':form})
    return render(request, 'quorascrap.html',context={'form':form})

def mediumscrap(request):
    form = ScrapForm(request.POST)
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            ...
            print("Medium")
            scrapMedium(un)
            return render(request, 'mediumscrap.html',context={'form':form})
        return render(request, 'mediumscrap.html',context={'form':form})
    return render(request, 'mediumscrap.html',context={'form':form})

def pinterestscrap(request):
    form = ScrapForm(request.POST)
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            ...
            print(type(un))
            scrapPinterest(un)
            return render(request, 'pinterestscrap.html',context={'form':form})
        return render(request, 'pinterestscrap.html',context={'form':form})
    return render(request, 'pinterestscrap.html',context={'form':form})

def combinedscrap(request):
    form = CombinedForm(request.POST)
    if request.method == 'POST':
        form = CombinedForm(request.POST)
        if form.is_valid():
            fb_un = form.cleaned_data['facebook_un']
            tw_un = form.cleaned_data['twitter_un']
            gh_un = form.cleaned_data['github_un']
            in_un = form.cleaned_data['instagram_un']
            lk_un = form.cleaned_data['linkedin_un']
            qu_un = form.cleaned_data['quora_un']
            me_un = form.cleaned_data['medium_un']
            pi_un = form.cleaned_data['pinterest_un']
            r=list([fb_un, tw_un, gh_un, in_un, lk_un, qu_un, me_un, pi_un])
            finaldata(r)
            return render(request, 'combined.html',context={'form':form})
        return render(request, 'combined.html',context={'form':form})
    return render(request, 'combined.html',context={'form':form})

def emaildata(request):
    headers = {"X-API-KEY": "62e89034-112f-4a13-909d-23705c4e4e72"}
    form = ScrapForm(request.POST)
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            ...
            r = requests.get(
                "https://api.seon.io/SeonRestService/email-api/v2.2/" + email,
                headers=headers)
            profile = json.loads(r.text)
            acc_data = profile['data']['account_details']
            keys = list(acc_data.keys())
            values = list(acc_data.values())
            Dict = {}
            for i in range(len(keys)):
                Dict[keys[i]] = values[i]['registered']
            print(Dict)
        ...
        return render(request, 'emaildata.html', context={'form': form})
    return render(request, 'emaildata.html', context={'form': form})
...
def phonedata(request):
    ...
    if request.method == 'POST':
        form = ScrapForm(request.POST)
        if form.is_valid():
            phoneno = form.cleaned_data['username']
            r = requests.get(
                "https://api.seon.io/SeonRestService/phone-api/v1.4/" +
                phoneno,
                headers=headers)
            profile = json.loads(r.text)
            acc_data = profile['data']['account_details']
        ...
        return render(request, 'phonedata.html', context={'form': form})
    return render(request, 'phonedata.html', context={'form': form})
...
def bindata(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            phoneno = form.cleaned_data['username']
            r = requests.get(
                "https://api.seon.io/SeonRestService/bin-api/v1.0/520000",
                headers=headers)
            profile = json.loads(r.text)
            acc_data = profile['data']
            keys = acc_data.keys()
            return render(request,
                          'bindata.html',
                          context={
        ...
        return render(request, 'bindata.html', context={'form': form})
    return render(request, 'bindata.html', context={'form': form})
...
def send_email(mail, subject,temp_message):
    ...
    message['From'] = 'policeHackathon23@gmail.com'
    message['To'] = mail
    ...
    try:
        ...
        mail_server.login(user="policeHackathon23@gmail.com", password="Police@123")
        mail_server.sendmail("policeHackathon23@gmail.com", mail, msg=message.as_string())
        mail_server.quit()
