...
from utils.utils import check_if_whatsapp_exists
...
def getDetailsFromPhoneNumber(phoneNumber: str) -> dict:
    print(phoneNumber)
    '''
    ...
    truecaller_res = numsearch(phoneNumber)

    address_details = ""
    try:
        address_details = truecaller_res['address'][0]['address']
    except Exception as e:
        address_details = ""


    google_ts_res_plain = search_text(truecaller_res['name'], no_of_results=10)
    google_ts_res_email = search_text(truecaller_res['email'], no_of_results=10)
    google_ts_res_email_name = search_text(str.split(truecaller_res['email'], '@')[0], no_of_results=10)
    google_ts_res_address = search_text(truecaller_res['name'] + address_details, no_of_results=10)
    google_ts_res_facebook = search_text("site:facebook.com " + truecaller_res['name'], no_of_results=5)
    google_ts_res_twitter = search_text("site:twitter.com " + truecaller_res['name'], no_of_results=5)
    google_ts_res_instagram = search_text("site:instagram.com " + truecaller_res['name'], no_of_results=5)
    google_ts_res_linkedin = search_text("site:linkedin.com " + truecaller_res['name'], no_of_results=5)
    ...
    google_ts_res_zabuacorp = search_text(truecaller_res['name'] + " Zaubacorp", no_of_results=5)
    google_ts_res_indiakanoon = search_text(truecaller_res['name'] + " IndiaKanoon", no_of_results=5)

    if(len(twitter_res) > 0):
        top_tweet_desc = twitter_res[0]['user_description']
        entity_context = extract_entity_string(top_tweet_desc)
        google_ts_entity_res_facebook = search_text("site:facebook.com " + truecaller_res['name'] + " " + entity_context, no_of_results=5)
        google_ts_entity_res_twitter = search_text("site:twitter.com " + truecaller_res['name'] + " " + entity_context, no_of_results=5)
        google_ts_entity_res_instagram = search_text("site:instagram.com " + truecaller_res['name'] + " " +entity_context, no_of_results=5)
        google_ts_entity_res_linkedin = search_text("site:linkedin.com " + truecaller_res['name'] + " " + entity_context, no_of_results=5)
        google_is_entity_res_facebook = search_image("site:facebook.com " + truecaller_res['name'] + " " + entity_context, no_of_results=5)
        google_is_entity_res_twitter = search_image("site:twitter.com " + truecaller_res['name'] + " " + entity_context, no_of_results=5)
        google_is_entity_res_instagram = search_image("site:instagram.com " + truecaller_res['name'] + " " +entity_context, no_of_results=5)
        google_is_entity_res_linkedin = search_image("site:linkedin.com " + truecaller_res['name'] + " " + entity_context, no_of_results=5)
        google_is_res_entity = search_image(truecaller_res['name'] + " " + entity_context, no_of_results=5)
        google_is_res_name = search_image(truecaller_res['name'], no_of_results=5)
...
def get_whatsapp_details(data):
    phone_number = data["phoneNumber"]
    if(check_if_whatsapp_exists(phone_number)):
        return {
            'isAvailable': True,
...
def generate_response(data):
    ...
    result = {}
    result['name'] = data['truecaller']['name']
    result['gender'] = data['truecaller']['gender'] if data['truecaller']['gender'] != "UNKNOWN" else d.get_gender(u"Bob")
    result['email'] = data['truecaller']['email']
    ...
    result['financialFootprint'] = get_validity_for_all_vpaIds(data["phoneNumber"])
    result['whatsappDetails'] = get_whatsapp_details(data)
    # result['availableApps'] = get_available_apps(data)
...
'''
    ...
    address
'''
def getDetailsFromPhoneNumberForTwilio(phoneNumber: str) -> dict:
    phoneNumberWith91 = "+91" + phoneNumber

    print("test1")
    truecaller_res = numsearch(phoneNumberWith91)
    google_ts_res_twitter = search_text("site:twitter.com " + truecaller_res['name'], no_of_results=1)
 
    filtered_username = get_twitter_usernames(google_ts_res_twitter).pop()
    twitter_res = get_user_information(filtered_username)

    google_ts_entity_res_facebook = search_text("site:facebook.com " + truecaller_res['name'] , no_of_results=5)
    google_ts_entity_res_linkedin = search_text("site:linkedin.com " + truecaller_res['name'] , no_of_results=5)
    print("test4") 
    vpa_data = get_validity_for_all_vpaIds(phoneNumber)
    data = {
        'name':truecaller_res['name'],
        'email':truecaller_res['email'],
        'carrier':truecaller_res['carrier'],
        'facebook':google_ts_entity_res_facebook[0]['url'],
        'linkedin':google_ts_entity_res_linkedin[0]['url'],
        'twitter':"https://www.twitter.com/" + twitter_res['user_screen_name'],
        'location':twitter_res['user_location'],
        'email':truecaller_res['email'],
        'availableOnPhonePe': "Yes" if vpa_data['phonepe']['isAvailable'] else "No",
        'availableOnPaytm' : "Yes" if vpa_data['paytm']['isAvailable'] else "No"
    }
    str = "*Found the below details:* \n\n"
    for key in data:
        str += '*' + key.capitalize() + '*' + ": " + data[key] + '\n'
    return str
